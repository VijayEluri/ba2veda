...
public class XMLWriter
{
	...
	/**
	 * Determine whether to output comment.
	 */
	 ...
	 * Writes an XML element.
	 *
	 * @param includeComment Determine whether to generate comment.
	 */
			...
			throws IOException
	{
		...
		if (prettyPrint)
		{
			...
			_indentS = _indent.toString();
			this.writer.print(_indentS);
		}
		...
		{
			// Add comment
			if (includeComment && xml.getComment() != null &&
					xml.getComment().trim().length() != 0)
			{
				String _commentTag = "<!--" + xml.getComment() + "-->";
				if (prettyPrint)
				{
					this.writer.println(_commentTag);
					this.writer.print(_indentS);
				}
				else
				{
					this.writer.print(_commentTag);
				}
			}

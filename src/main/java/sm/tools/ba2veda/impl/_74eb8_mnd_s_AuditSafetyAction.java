			...
			throws Exception
	{
		...
		for (XmlAttribute att : atts)
		{
			...
			if (code.equals("Мероприятие для предотвращения"))
				description1 = ba_field_to_veda(att, uri, ba_id, doc, path, parent_ba_doc_id, parent_veda_doc_uri, true);
			else if (code.equals("Мероприятие незамедлительное"))
				description2 = ba_field_to_veda(att, uri, ba_id, doc, path, parent_ba_doc_id, parent_veda_doc_uri, true);
			if (code.equals("auto_num"))
				registrationNumber = ba_field_to_veda(att, uri, ba_id, doc, path, parent_ba_doc_id, parent_veda_doc_uri, true);
			else if (code.equals("Подразделение"))
				responsibleDepartment = ba_field_to_veda(att, uri, ba_id, doc, path, parent_ba_doc_id, parent_veda_doc_uri, true);
			else if (code.equals("date_to"))
				datePlan = ba_field_to_veda(att, uri, ba_id, doc, path, parent_ba_doc_id, parent_veda_doc_uri, true);

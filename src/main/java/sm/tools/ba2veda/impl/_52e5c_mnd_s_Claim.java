...
public class _52e5c_mnd_s_Claim extends Ba2VedaTransform {
	public _52e5c_mnd_s_Claim(BaSystem _ba, VedaConnection _veda, Replacer replacer) {
		super(_ba, _veda, replacer, "52e5c18882714257863eb97b690491cb", "mnd-s:Claim");
	}

	public void inital_set() {
		fields_map.put("cmt_number", "mnd-s:registrationNumberCMT");
			...
			if (predicate != null) {
				...
				if (code.equals("Тема"))
					...
					comment.addProperty("v-s:attachment", rss);
						...
						|| code.equals("Связанный документ"))
				{
					if (comment == null) 
						comment = new Individual();
					...
					if (has_letter_registration_record_recipient == null) {
						...
						has_letter_registration_record_recipient.setUri(uri + 
								"_letter_registration_record_recipient");
					...
					if (has_letter_registration_record_sender == null) {
						...
						has_letter_registration_record_sender.setUri(uri + 
								"_letter_registration_record_sender");
					...
					new_individual.addProperty("mnd-s:hasClaimReport", rss);
				} else if (code.equals("other_sides")) {
					new_individual.addProperty("v-s:stakeholder", 
							veda.getIndividual("d:" + att.getLinkValue()).getResources("v-s:backwardTarget"));
				}

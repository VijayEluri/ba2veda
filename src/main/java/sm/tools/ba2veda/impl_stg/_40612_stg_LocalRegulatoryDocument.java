...
public class _40612_stg_LocalRegulatoryDocument extends Ba2VedaTransform
{
	public _40612_stg_LocalRegulatoryDocument(BaSystem _ba, VedaConnection _veda, Replacer replacer)
	{
		super(_ba, _veda, replacer, "406127428bba4a4da11eb8b3c5e8a4b8", "stg:LocalRegulatoryDocument");
	}
			...
			throws Exception
	{
		...
		new_individual.addProperty("v-s:hasDocumentKind", "d:c9e0a1dd43d943ed87f5072a6790d5c2", Type._Uri);
		new_individual.addProperty("v-s:owner", "d:org_RU1121016110_1", Type._Uri);
		new_individual.addProperty("v-s:hasSector", "d:9d2c61a621634fdc8e9d889869c568f1", Type._Uri);

		Individual version = new Individual();
		version.setUri(new_individual.getUri() + "_1");
		version.addProperty("v-s:hasDocumentKind", "d:c9e0a1dd43d943ed87f5072a6790d5c2", Type._Uri);
		...
		for (XmlAttribute att : atts)
		{
			...
			if (predicate != null)
			{
				...
				} else if (code.equals("department"))
				{
					...
					new_individual.addProperty("v-s:initiator", rss);
				...
				else if (code.equals("profession"))
				{
					version.addProperty("stg:hasPositionDictionary", rss);
					new_individual.addProperty("stg:hasPositionDictionary", rss);

					Individual iii = veda.getIndividual(rss.resources.get(0).getData());
					if (iii != null)
					{
						Resources rr = iii.getResources("rdfs:label");
						if (rr != null)
						{
							new_individual.addProperty("v-s:title", rr);
							version.addProperty("v-s:title", rr);
						}
					}
				}
			}
